...
async function fetchRandomMusic() {
    ...
    } while (keyword === state.lastKeyword && state.radarKeywords.length > 1);
    state.lastKeyword = keyword;
    
    try {
        // 使用现有的API.search方法搜索音乐
        const results = await API.search(keyword, state.searchSource, 50);
        
        // 处理搜索结果
        state.onlineSongs = results;
        ...
        // 显示通知
        showNotification(`探索雷达: 正在播放「${keyword}」主题音乐`);
    } catch (error) {
        console.error('获取随机音乐失败:', error);
        showNotification('获取随机音乐失败，请稍后重试', 'error');
    } finally {
        // 隐藏加载状态
        if (dom.loadOnlineBtn) {
            const btnText = dom.loadOnlineBtn.querySelector('.btn-text');
...
async function fetchChineseMusic() {
    ...
    try {
        const results = await API.search(category, state.searchSource, 50);
        ...
        // 显示通知
        showNotification(`华语音乐: 正在播放「${category}」`);
    } catch (error) {
        console.error('获取华语音乐失败:', error);
        showNotification('获取华语音乐失败，请稍后重试', 'error');
    } finally {
        // 隐藏加载状态
        if (dom.loadOnlineBtn) {
            const btnText = dom.loadOnlineBtn.querySelector('.btn-text');
